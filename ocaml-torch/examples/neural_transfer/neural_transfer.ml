...
let load_pretrained_vgg weight_path cpu =
  ...
  let layers = List.sort_uniq compare (layers_style_loss @ layers_content_loss) in
  let max_layer = 1 + List.(rev layers |> hd) in
  let pretrained_model =
    Vgg.vgg16_layers model_paras ~max_layer ~batch_norm:false |> Base.Staged.unstage
  in
  let _ =
...
let load_style_img style_img cpu =
  Imagenet.load_image_no_resize_and_crop style_img |> Tensor.to_device ~device:cpu

let load_content_img content_img cpu =
  Imagenet.load_image_no_resize_and_crop content_img |> Tensor.to_device ~device:cpu
...
let get_inputs_tensors cpu argv =
  let style_img_path, content_img_path, weight_path =
    ...
    | _ ->
      raise (Failure "Must input: style_img.png content_img.png pretrained_weight.ot")
  in
...
let get_style_loss input_layers style_layers layers_for_loss =
  let style_loss m1 m2 = Tensor.mse_loss (gram_matrix m1) (gram_matrix m2) in
  let loss_new_layers lst =
    style_loss (Base.Map.find_exn input_layers lst) (Base.Map.find_exn style_layers lst)
  in
...
let training_nst model input_var optimizer style_layers content_layers total_steps =
  for iteration = 1 to total_steps do
    Optimizer.zero_grad optimizer;
    let input_layers = model input_var in
    let style_loss = get_style_loss input_layers style_layers layers_style_loss in
    let content_loss = get_content_loss input_layers content_layers layers_content_loss in
    ...
    let _ =
      Stdio.printf
        "Iteration: %d,  Combined Loss: %.4f, Style Loss: %.4f, Content Loss: %.4f\n%!"
        iteration
